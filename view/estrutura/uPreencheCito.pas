...
uses
  ...
  Vcl.Buttons, Vcl.Mask, wwdbedit, Wwdotdot, Wwdbcomb, Vcl.ExtCtrls, wwdblook;
...
type
  ...
  TfrmPreencheCito = class(TfrmTmpFormNormal)
    ...
    cbAtrofia: TCheckBox;
    cbAbrir: TCheckBox;
    cbNegativo: TCheckBox;
    cbPadrao: TwwDBLookupCombo;
...
uses uExameLaudo, dataExames, dataLookups;
...
//--- Monta laudo --------------------------------------------------------------
  ...
  //Verifica padrões
  if dmLookups.lkProcLaudosPreModlpm_codigo.Value <> 0 then
    vFraseLaudo01 := vFraseLaudo01 + TEncoding.ANSI.GetString(dmLookups.lkProcLaudosPreModlpm_modelo.Value);
...
//------------------------------------------------------------------------------
  ...
  if cbOcasionais.Checked = true then
    vFraseLaudo02 := vFraseLaudo02 + ' e ocasionais hemácias'
  else
    if cbEsparsas.Checked = true then
      vFraseLaudo02 := vFraseLaudo02 + ' e esparsas hemácias'
    else
...
//-------------------------------------------------------------------------------------------------------------------------
  ...
  if dmLookups.lkProcLaudosPreModlpm_codigo.Value <> 0 then
  begin
    if not(dmLookups.lkProcLaudosPreModlpm_diagnostico.IsNull) then
      vFraseDiag01 := vFraseDiag01 + TEncoding.ANSI.GetString(dmLookups.lkProcLaudosPreModlpm_diagnostico.Value);
  end;
  ...
  if cbNegativo.Checked = true then
    vFraseDiag01 := vFraseDiag01 + '. Negativo para lesão intraepitelial e malignidade';
...
begin
  ...
  edObs.Clear;
  edObs.Enabled := false;
  cbPadrao.Text := dmLookups.lkProcLaudosPreModlpm_descricao.Text;
end;
